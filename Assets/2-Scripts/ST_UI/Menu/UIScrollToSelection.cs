...
namespace UnityEngine.UI.Extensions
{
    ...
    public class UIScrollToSelection : MonoBehaviour
    {
        ...
        {
            // move the current scroll rect to correct position
            float selectionPosition = -selection.anchoredPosition.x - (selection.rect.width * (1 - selection.pivot.x) - XPadding);
            ...
            float normalizedPosition = TargetScrollRect.horizontalNormalizedPosition + (offlimitsValue / LayoutListGroup.rect.width);

            if(offlimitsValue < 0) 
            { 
                normalizedPosition -= (Mathf.Abs(offlimitsValue) / LayoutListGroup.rect.width); 
            }
            else if (offlimitsValue > 0) 
            { 
                normalizedPosition += (offlimitsValue / LayoutListGroup.rect.width); 
            }

            normalizedPosition = Mathf.Clamp01(normalizedPosition);

            // move the target scroll rect
            TargetScrollRect.horizontalNormalizedPosition = Mathf.SmoothStep(TargetScrollRect.horizontalNormalizedPosition, normalizedPosition, Time.unscaledDeltaTime * scrollSpeed);
        }
